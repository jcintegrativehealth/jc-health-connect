...
const STATUS_TO_DB: Record<Status, "pending" | "approved" | "rejected"> = {
  Pending: "pending",
  Approved: "approved",
  Rejected: "rejected",
};

function toRow(record: CommentRecord): C {
  const ageHours = Math.max(0, (Date.now() - new Date(record.createdAt).getTime()) / 3_600_000);
  const date =
    ageHours < 1 ? "now" : ageHours < 24 ? `${Math.floor(ageHours)}h` : `${Math.floor(ageHours / 24)}d`;
  return {
    id: record.id,
    author: record.authorName,
    article: record.articleSlug,
    date,
    ageHours,
    body: record.body,
    status: (record.status.charAt(0).toUpperCase() + record.status.slice(1)) as Status,
  };
}
...
function Comments() {
  ...
  useEffect(() => {
    ...
    listCommentsForModerationFn()
      ...
      .catch((err) => {
        if (cancelled) return;
        setLoadError(err instanceof Error ? err.message : "Could not load the moderation queue.");
        setLoading(false);
      });
    return () => {
      cancelled = true;
  ...
  const runAction = async (comment: C, next: Status) => {
    ...
    try {
      await moderateCommentFn({ data: { id: comment.id, status: STATUS_TO_DB[next] } });
      ...
      toast.success(ACTION_COPY[next].toast, {
        ...
        action: {
          label: "Undo",
          onClick: () => {
            moderateCommentFn({ data: { id: comment.id, status: STATUS_TO_DB[prevStatus] } })
              .then(() => {
        ...
        <ul className="divide-y divide-navy/10 -m-4">
          {loading && (
            <li className="p-12 text-center text-sm text-navy/45">
              <Loader2 size={14} className="inline animate-spin mr-2" /> Loading moderation queue…
            </li>
          )}
          {!loading && loadError && (
            <li className="p-12 text-center text-sm text-navy/60">{loadError}</li>
          )}
